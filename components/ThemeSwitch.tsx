  ...
  return (
    <div className="flex flex-row items-center space-x-3">
      <svg width="17" height="16" xmlns="http://www.w3.org/2000/svg">
        <path
          className={enabled ? "fill-white" : "fill-[#5A6069]"}
      ...
      <Switch
        ...
        onChange={setEnabled}
        className={` relative
          inline-flex h-[24px] w-[48px] shrink-0 cursor-pointer rounded-full border-2 border-transparent bg-[#5A6069] transition-colors duration-200 ease-in-out focus:outline-none focus-visible:ring-2  focus-visible:ring-white focus-visible:ring-opacity-75`}
      >
        <span className="sr-only">Use setting</span>
        <span
          aria-hidden="true"
          className={`${enabled ? "translate-x-1" : "translate-x-7"}
            pointer-events-none  inline-block h-[12px] w-[12px] translate-y-1 transform rounded-full bg-white shadow-lg ring-0 transition duration-200 ease-in-out`}
        />
      </Switch>
      <svg width="18" height="18" xmlns="http://www.w3.org/2000/svg">
        <path
          className={enabled ? "fill-[#5A6069]" : "fill-white"}
