...
export default function Header({
  ...
  const { title, setTitle, content } = useTitle();

  const { isLoading: isCreating, error, mutate } = useCreateDocument();
  const {
    isLoading: isUpdating,
    error: updateError,
    mutate: updateMutate,
  } = useUpdateDocument(router.query.id as string);
  ...
  //   ) {
  //     const document = data?.user?.documents.find(
  ...
  const handleSave = () => {
    if (router.pathname === "/document/new") {
      mutate({ title, content });
    }

    if (router.pathname === "/document/[id]") {
      updateMutate({ title, content });
    }
  };

  return (
    <div className="flex w-full flex-row">
      <div className=" flex h-14 w-14 items-center justify-center bg-[#35393F] p-4 hover:bg-[#E46643] sm:h-[72px] sm:w-[72px]">
        <button type="button" onClick={() => setSidebarOpen(!sidebarOpen)}>
          {sidebarOpen ? (
            ...
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-[18px] w-[23px] sm:h-[18px] sm:w-[30px]"
            >
              <g fill="#FFF" fillRule="evenodd">
      ...
      <div className="flex w-full flex-row items-center bg-[#2B2D31] py-1  sm:py-2">
        <div className="flex w-full flex-row items-center sm:space-x-6 ">
          ...
          <div className="hidden sm:block sm:h-[40px] sm:w-[1px] sm:bg-[#5A6069]"></div>
          <div className="px-4 sm:px-0">
            <FileRename name={title} onChange={setTitle} />
          </div>
        </div>
        <div className="flex flex-row items-center space-x-4 px-4">
          <div>
            <SaveModal
              documentId={router.query.id as string}
              fileName={title}
